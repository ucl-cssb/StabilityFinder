...
for(i in 1:numb_files){
  pltList[[i]] <-ggplot(data_list[[i]], aes(x=V1, y=V2)) + geom_point() + xlim(0,50) + ylim(0,50) + theme(axis.title.x = element_blank(),
                                                                                                           axis.title.y = element_blank())

}
g <- do.call("arrangeGrob", c(pltList, list(ncol=sqrt(numb_files) ,sub=textGrob("A2",gp = gpar(fontsize=20, fontface="bold", fontsize=20)), left=textGrob("B2", rot = 90, vjust = 1,gp = gpar(fontsize=20, fontface="bold", fontsize=20)),main=textGrob(title_text, vjust =1, gp = gpar(fontsize=20, fontface="bold", fontsize=20)))))
ggsave(file=paste(filename,".pdf",sep=''), g, scale=3)
}
setwd("/Volumes/cblin2/work/scanning_ABC_SMC/results_txt_files/Population_4")
plot_stabilityChecker_particles(100, "MySwitch_pop5_result","Steady state values of the particles in the last population of the Gardner bistable switch")
...
plot_posterior_distr <- function(numb_params, param_names, limits, file_name,plot_title){
  ...
  for(i in 1:numb_params)
    for(j in 1:numb_params){
      k=k+1
      if(i==j){
        pltList[[k]] <-ggplot(p_values_final, aes_string(x=param_names[i], weight=param_names[11])) + geom_density(fill="grey") + xlim(a[i],b[i])+ ggtitle(param_names[i]) +
        ...
        pltList[[k]] <-ggplot(p_values_final, aes_string(x = param_names[i], y = param_names[j], weight=param_names[ncol(p_values_final)])) + 
...
plot_posterior_distr(10, param_names, limits, file_name, plot_title)
